...
with DAG(dag_id="silver_players_matches_ingestion"
         ,default_args=default_args
         ,description='DAG responsável por fazer a ingestão das partidadas dos players BRONZE.'
    ...
    with TaskGroup(
      ...
      match_players_r = SparkSubmitOperator(
                              task_id=f'SILVER_PLAYERS_MATCHES',
                              conn_id='spark_local',
    ...
    with TaskGroup(
      ...
      match_players_b = SparkSubmitOperator(
                              task_id=f'SILVER_PLAYES_MATCHS_to_BRONZE_LAYER',
                              conn_id='spark_local',
                              application_args=['SILVER'],
                              jars='/usr/local/airflow/jars/hadoop-azure-3.2.1.jar,\
                                    /usr/local/airflow/jars/hadoop-common-3.3.2.jar,\
    ...
    with TaskGroup(
      ...
      match_players_g = SparkSubmitOperator(
                              task_id=f'SILVER_PLAYES_MATCHS_to_GOLD_LAYER',
                              conn_id='spark_local',
                              application_args=['SILVER'],
                              jars='/usr/local/airflow/jars/hadoop-azure-3.2.1.jar,\
                                    /usr/local/airflow/jars/hadoop-common-3.3.2.jar,\
