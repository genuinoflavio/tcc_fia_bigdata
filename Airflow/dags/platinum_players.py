...
with DAG(dag_id="platinum_players_ingestion"
         ,default_args=default_args
         ,description='DAG responsável por fazer a ingestão dos players PLATINA.'
    ...
    with TaskGroup(
      ...
      platina_players_b = SparkSubmitOperator(
                                    task_id=f'PLATINA_PLAYERS_to_BRONZE_LAYER',
                                    conn_id='spark_local',
                                    ...
                                    name=f'task_id',
                                    application='/usr/local/airflow/dags/spark_scripts/bronze/platina_players_raw_to_bronze.py',
    ...
    with TaskGroup(
      ...
      platina_players_s = SparkSubmitOperator(
                                    task_id=f'PLATINA_PLAYERS_to_SILVER_LAYER',
                                    conn_id='spark_local',
                                    jars='/usr/local/airflow/jars/hadoop-azure-3.2.1.jar,\
                                          /usr/local/airflow/jars/hadoop-common-3.3.2.jar,\
                                          /usr/local/airflow/jars/hadoop-aws-3.3.2.jar,\
                                    ...
                                    executor_memory='250m',
                                    name=f'task_id',
                                    application='/usr/local/airflow/dags/spark_scripts/silver/platina_players_bronze_to_silver.py',
                                    execution_timeout=timedelta(minutes=180),
                                    dag=dag
                              )
    ...
    with TaskGroup(
      ...
      platinum_players_g = SparkSubmitOperator(
                              task_id=f'PLATINUM_PLAYERS_to_GOLD_LAYER',
                              conn_id='spark_local',
                              jars='/usr/local/airflow/jars/hadoop-azure-3.2.1.jar,\
                                    /usr/local/airflow/jars/hadoop-common-3.3.2.jar,\
                                    /usr/local/airflow/jars/hadoop-aws-3.3.2.jar,\
                              ...
                              executor_memory='250m',
                              name=f'task_id',
                              application='/usr/local/airflow/dags/spark_scripts/gold/platinum_players_silver_to_gold.py',
                              execution_timeout=timedelta(minutes=180),
                              dag=dag
                        )
...
start_dag >> raw_ingestion >> \
      bronze_ingestion  >> platina_players_b >> \
            silver_ingestion >> [platina_players_s] >> \
                  gold_ingestion >> platinum_players_g  >> dag_finish